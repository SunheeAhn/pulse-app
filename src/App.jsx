...
function SettingsModal({ onClose, C }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"#000000aa", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:1000, padding:20 }} onClick={onClose}>
      <div style={{ background:C.surface, borderRadius:"24px 24px 0 0", border:`1px solid ${C.border}`, padding:"20px 0", width:"100%", maxWidth:480 }} onClick={e=>e.stopPropagation()}>
        <div style={{ fontWeight:800, fontSize:18, color:C.text, textAlign:"center", marginBottom:16 }}>설정</div>
        <div style={{ display:"flex", flexDirection:"column" }}>
          {[ "앱 및 웹사이트", "QR 코드", "알림", "설정 및 개인정보", "Meta Verified", "관리 감독", "로그인 활동", "로그아웃" ].map((item, i) => (
            <button key={i} style={{ background:"none", border:"none", cursor:"pointer", padding:"14px 20px", color:C.text, fontSize:16, textAlign:"left", borderBottom:i < 7 ? `1px solid ${C.border}` : "none" }}>
              {item}
            </button>
          ))}
          <button onClick={onClose} style={{ background:"none", border:"none", cursor:"pointer", padding:"14px 20px", color:C.muted, fontSize:16, textAlign:"left", fontWeight:700, marginTop:10 }}>취소</button>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ padding:"0 16px", paddingTop:8 }}>
        ...
        {tab==="profile" && (
          <div style={{ paddingTop:16 }}>
            ...
            <button onClick={()=>setShowSettings(true)} style={{ width:"100%", background:C.surfaceHover, border:`1px solid ${C.border}`, borderRadius:14, padding:13, color:C.text, fontWeight:700, fontSize:14, cursor:"pointer", marginTop:20 }}>설정</button>
            <button onClick={()=>setLoggedIn(false)} style={{ width:"100%", background:C.surfaceHover, border:`1px solid ${C.border}`, borderRadius:14, padding:13, color:"#ef4444", fontWeight:700, fontSize:14, cursor:"pointer", marginTop:10 }}>로그아웃</button>
          </div>
        )}
      </div>
